...
const AllPlayersPopup: React.FC<AllPlayersPopupProps> = ({ teams, onClose }) => {
  // Get all players from all teams (filter undefined)
  const allPlayers = teams.flatMap(team =>
    team.players
      ? team.players.map(player => ({
          ...player,
            ...
            <div className="teams-summary">
              <div className="summary-stats">
                ...
                <div className="stat-item">
                  ...
                  <span className="stat-value">
                    {
                      Object.keys(playersByTeam).filter(
                ...
                {teams.map(team => (
                  <div key={team.id} className="team-section">
                    <div className="team-header">
                      ...
                      <span className="owner">Owner: {team.owner}</span>
                      <span className="player-count">
                        {team.players ? team.players.length : 0} player
                        {team.players && team.players.length !== 1 ? 's' : ''}
                      </span>
                    </div>
                      ...
                      <div className="players-grid">
                        {team.players.map(player => (
                          <div key={player.id} className="player-card">
                            <div className="player-photo">
                              <img
                                src={player.imgUrl || player.imgUrl || '/default-player.png'}
                                alt={player.name}
                              />
                            </div>
                            <div className="player-info">
